...
import { useProjects, deductTokenAction } from "@/frontend/store/store";
    ...
    setTimeout(() => {
      ...
      setGeneratedPlan({
        ...
        featuresInScope: [
          ...
          "Edge cases defined in requirements",
        ],
        featuresNotInScope: [
        ...
        testingApproach:
          testingTypes.length > 0
            ? testingTypes.join(", ") + " testing approach."
            : "Functional testing.",
        passFail:
          "Pass: All critical test cases execute successfully. Fail: Any P0/P1 bugs discovered.",
        environment: environment || "Not specified",
        risks: includeRisks
          ? [
              {
                risk: "Data loss during timeout",
                mitigation: "Implement robust retry and fallback logic",
              },
              {
                risk: "Third-party API limit reached",
                mitigation: "Use mock responses for 80% of test runs",
              },
            ]
          : null,
      });
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader
        ...
        subtitle="Generate a test plan and its test cases from a requirement, spec, or user story."
      />
        ...
        <div className="space-y-8">
                  ...
                  >
                    {file ? (
                      ...
                      <>
                        ...
                        <p className="mt-3 font-display text-lg">Drop a file</p>
                      </>
                    )}
                  </div>
                  <input
          ...
          <section className="border border-border bg-card p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <label htmlFor="includeRisks" className="text-sm">
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="flex justify-end gap-3 mb-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="w-56 bg-card border border-border shadow-lg font-sans"
              >
                <DropdownMenuItem
                  className="text-sm cursor-pointer hover:bg-muted p-2"
                  onClick={() => {
          ...
          <div id="planner-document-container" className="border border-border bg-card p-8">
            ...
            <div className="space-y-6 text-sm">
              <div>
                <h4 className="font-bold text-foreground mb-1">1. Scope</h4>
                <p className="text-muted-foreground">{generatedPlan.scope}</p>
              </div>
              <div>
                <h4 className="font-bold text-foreground mb-1">2. Objectives</h4>
                <ul className="list-disc pl-5 text-muted-foreground">
                  {generatedPlan.objectives.map((o: string, i: number) => (
                    <li key={i}>{o}</li>
                  ))}
                </ul>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h4 className="font-bold text-foreground mb-1">3. Features to be Tested</h4>
                  <ul className="list-disc pl-5 text-muted-foreground">
                    {generatedPlan.featuresInScope.map((f: string, i: number) => (
                      <li key={i}>{f}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h4 className="font-bold text-foreground mb-1">4. Features NOT in Scope</h4>
                  <ul className="list-disc pl-5 text-muted-foreground">
                    {generatedPlan.featuresNotInScope.map((f: string, i: number) => (
                      <li key={i}>{f}</li>
                    ))}
                  </ul>
                </div>
              ...
              <div>
                <h4 className="font-bold text-foreground mb-1">5. Testing Approach</h4>
                <p className="text-muted-foreground">{generatedPlan.testingApproach}</p>
              </div>
              <div>
                <h4 className="font-bold text-foreground mb-1">6. Pass / Fail Criteria</h4>
                <p className="text-muted-foreground">{generatedPlan.passFail}</p>
              </div>
              <div>
                <h4 className="font-bold text-foreground mb-1">7. Test Environment</h4>
                <p className="text-muted-foreground">{generatedPlan.environment}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <input
                      type="text"
                      className="border-b border-dashed border-border bg-transparent outline-none focus:border-accent"
                      defaultValue="TBD"
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <span className="w-24">Prepared by:</span>
                    <input
                      type="text"
                      className="border-b border-dashed border-border bg-transparent outline-none focus:border-accent"
                      defaultValue={tester || "QA"}
                    />
                  </div>
                  <div className="flex items-center gap-2 mt-1 text-muted-foreground">
                    <span className="w-24">Reviewed by:</span>
                    <input
                      type="text"
                      className="border-b border-dashed border-border bg-transparent outline-none focus:border-accent"
                      defaultValue=""
