    ...
    <div className="mx-auto max-w-5xl space-y-8">
              ...
              <div className="flex items-center gap-3 rounded-[8px] border border-[var(--c-border)] bg-[var(--c-bg-input)]/25 p-4 mt-4">
                ...
                <label className="relative inline-flex cursor-pointer items-center">
                  <input
                    ...
                    onChange={(e) => setPrefCoverage(e.target.checked)}
                    className="peer sr-only"
                  />
                  <div className="peer h-6 w-11 rounded-full bg-[var(--c-border-strong)] after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:bg-white after:transition-all peer-checked:bg-[var(--c-accent)] peer-checked:after:translate-x-full"></div>
                </label>
      ...
      <div className="mt-8 pt-8 border-t border-[var(--c-border)] max-w-[600px]">
        ...
        <button
          ...
          className="rounded-[6px] border border-[var(--c-border)] bg-[var(--c-bg-input)] px-4 py-2 text-[13px] font-medium text-[var(--c-text)] transition-colors hover:bg-[var(--c-bg-hover)]"
        >
...
function TeamMembersCard() {
  ...
  const [role, setRole] = useState("Developer"); // Job Title
  const [workspaceRole, setWorkspaceRole] = useState<WorkspaceRole>("Editor");

  const [members, setMembers] = useWorkspaceMembers();
  ...
  const handleEdit = (profile: any) => {
    ...
    setWorkspaceRole(wm ? wm.role : "Editor");
    setShowAddForm(true);
  };
    ...
    <div className="rounded-[12px] border border-[var(--c-border)] bg-[var(--c-bg-card)] p-6 space-y-4">
      ...
      {showAddForm ? (
        <form
          ...
          className="space-y-4 rounded-[8px] border border-[var(--c-border)] bg-[var(--c-bg-input)]/10 p-4"
        >
          <h4 className="font-medium text-[14px] text-[var(--c-text)]">
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-[var(--c-border)]/50">
            <button
              type="button"
              onClick={resetForm}
              className="rounded-[6px] border border-[var(--c-border)] bg-transparent px-3 py-1.5 text-[11px] font-medium text-[var(--c-text-muted)] hover:text-[var(--c-text)]"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="rounded-[6px] bg-[var(--c-text)] px-3 py-1.5 text-[11px] font-medium text-[var(--c-bg)] hover:opacity-90"
            >
        ...
        <div className="divide-y divide-[var(--c-border)]">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--c-accent)] font-semibold text-[13px] text-white">
                  ...
                  <div className="min-w-0">
                    <p className="text-[13px] font-medium text-[var(--c-text)] truncate flex items-center gap-2">
                      ...
                      {isSelf && (
                        <span className="text-[10px] font-normal text-[var(--c-text-muted)] font-mono bg-[var(--c-bg-hover)] border border-[var(--c-border)] px-1 py-0.2 rounded">
                          (you)
                        </span>
                      )}
                    </p>
                    <p className="text-[11px] text-[var(--c-text-muted)] font-mono truncate mt-0.5">
                      {p.role} · {p.email}
                    </p>
                  </div>
                </div>
          ...
          {profiles.length === 0 && (
            <p className="text-[13px] text-[var(--c-text-muted)] py-4 text-center">
