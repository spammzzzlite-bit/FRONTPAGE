      ...
      {suites.length === 0 ? (
        <EmptyState
          ...
          cta={
            projects.length > 0
              ? {
                  label: "Create a suite",
                  onClick: () => {
                    setShowNew(true);
                    setNewProjectId(projects[0]?.id || "");
                  },
                }
              : { label: "Create a project", to: "/projects" }
          }
        />
      ) : (
      ...
      {showNew && (
        <Modal onClose={() => setShowNew(false)} title="New test suite">
          <form onSubmit={handleCreate}>
            <label className="mb-2 block font-mono text-[11px] uppercase tracking-[0.08em] text-[var(--c-text-muted)] font-medium">
              Project
            </label>
            <select
              value={newProjectId}
...
function SuiteRow({
  ...
  const passedCount = testCases.filter((tc) => tc.lastRunStatus === "passed").length;
  const failedCount = testCases.filter((tc) => tc.lastRunStatus === "failed").length;
      ...
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 font-mono text-[11px] text-[var(--c-text-muted)]">
            ...
            {passedCount > 0 && (
              <>
                {" "}
                · <span className="text-[var(--c-sage)]">{passedCount} passed</span>
              </>
            )}
            {failedCount > 0 && (
              <>
                {" "}
                · <span className="text-[var(--c-rust)]">{failedCount} failed</span>
              </>
            )}
          </p>
        </div>
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-2 text-[var(--c-text-muted)] hover:text-[var(--c-text)] hover:bg-[var(--c-bg-hover)] rounded-[6px] transition-colors"
            title="Add test case"
          >
          ...
          <button
            ...
            className="p-2 text-[var(--c-text-muted)] hover:text-[var(--c-fail)] hover:bg-[rgba(196,85,26,0.1)] rounded-[6px] transition-colors"
            title="Delete"
          >
        ...
        <div className="border-t border-[var(--c-border)] bg-[var(--c-bg)]">
          {showAddCase && (
            <form
              ...
              className="flex items-center gap-3 border-b border-[var(--c-border)] bg-[var(--c-bg-hover)] px-5 py-3"
            >
              <input
                autoFocus
                value={newTitle}
          ...
          {testCases.length === 0 && !showAddCase ? (
            ...
            testCases.map((tc) => (
              <div
                key={tc.id}
                onClick={() =>
                  openPanel(<TestCasePanel testCase={tc} />, [
                    { label: "Suites" },
                    { label: projectName },
                    { label: suite.name },
                    { label: tc.title },
              ...
              >
                ...
                <span
                  className={`rounded-sm px-[8px] py-[3px] font-mono text-[9px] status-${tc.authorStatus || tc.status}`}
                >
                  {tc.authorStatus || tc.status}
                </span>
                <span
                  className={`rounded-sm px-[8px] py-[3px] font-mono text-[9px] badge-${tc.priority}`}
                >
                  {tc.priority}
                </span>
                <button
...
function TestCasePanel({ testCase }: { testCase: TestCase }) {
  ...
  const [form, setForm] = useState({
    ...
    status: tc.authorStatus || tc.status,
    tags: tc.tags.join(", "),
  });
    ...
    <div className="space-y-6 text-[14px]">
      <div>
        ...
        {editing ? (
          ...
          <h2 className="mt-2 font-display text-[28px] text-[var(--c-text)] leading-tight">
            {tc.title}
          </h2>
        )}
      </div>
      ...
      {!editing && (
        <div className="flex flex-wrap gap-2">
          <span
            className={`rounded-[4px] px-[10px] py-[4px] font-mono text-[10px] uppercase tracking-wider status-${tc.authorStatus || tc.status}`}
          >
            {tc.authorStatus || tc.status}
          </span>
          {tc.lastRunStatus && (
            <span
              className={`rounded-[4px] px-[10px] py-[4px] font-mono text-[10px] uppercase tracking-wider status-${tc.lastRunStatus}`}
            >
              {tc.lastRunStatus}
            </span>
          )}
          <span
            className={`rounded-[4px] px-[10px] py-[4px] font-mono text-[10px] uppercase tracking-wider badge-${tc.priority}`}
          >
            {tc.priority}
          </span>
          {tc.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-[4px] border border-[var(--c-border)] bg-[var(--c-bg-card)] px-[10px] py-[4px] font-mono text-[10px] text-[var(--c-text-muted)]"
            >
              {tag}
            </span>
          ))}
      ...
      {editing ? (
        ...
        <div className="space-y-6">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-[var(--c-text-muted)] mb-2 font-medium">
              Steps
            </p>
            <p className="text-[14px] text-[var(--c-text)] whitespace-pre-wrap leading-relaxed">
              {tc.steps || "No steps defined"}
            </p>
          </div>
