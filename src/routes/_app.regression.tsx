...
import { FlaskConical, Plus, Play, CheckCircle2, XCircle, Trash2, Tag, Coins } from "lucide-react";
import {
  useProjects,
  useSuites,
  useTestCases,
...
export const Route = createFileRoute("/_app/regression")({
  head: () => ({ meta: [{ title: "Regression — QAMind AI" }] }),
...
function RegressionPage() {
  ...
  function handleDelete(id: string) {
    if (confirm("Delete this regression suite?")) {
      if (!deductTokenAction("Delete regression suite")) return;
  ...
  function handleRun(suite: RegressionSuite) {
    ...
    if (!deductTokenAction(`Run regression suite "${suite.name}"`)) {
    ...
    addNotification(
      ...
      `Regression run ${run.id} finished: ${run.results.filter((r) => r.status === "passed").length}/${run.results.length} passed.`,
      "success",
    );
    toast.success(
      `Regression run ${run.id} — ${run.results.filter((r) => r.status === "passed").length}/${run.results.length} passed`,
    );
  }
  ...
  function handleRunFocused() {
    ...
    if (!deductTokenAction("Run focused regression tests")) {
      toast.error("Insufficient tokens to run regression.");
      return;
    }
    ...
    toast.success(
      `Regression run ${run.id} complete — ${run.results.filter((r) => r.status === "passed").length}/${run.results.length} passed`,
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mb-8 rounded-[12px] border border-[var(--c-border)] bg-[var(--c-bg-card)] p-6 shadow-[var(--shadow-sm)]">
        <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--c-accent)] font-medium mb-4">
      ...
      {regSuites.length === 0 ? (
        <EmptyState
          ...
          cta={
            testCases.length > 0
              ? { label: "Create one", onClick: () => setShowNew(true) }
              : { label: "Create test cases first", to: "/suites" }
          }
        />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {regSuites.map((suite, i) => {
            ...
            return (
              <div
                key={suite.id}
                className="stagger-item border border-border bg-card p-6"
              ...
              >
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {suite.tags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1 rounded-sm border border-border px-2 py-0.5 font-mono text-[10px]"
                    >
                ...
                <div className="mt-4 pt-3 border-t border-border">
                  ...
                  <div className="flex gap-1.5">
                    {health.map((passed, j) => (
                      <div
                        key={j}
                        className={`h-6 flex-1 flex items-center justify-center rounded-sm ${passed ? "bg-sage/15" : "bg-rust/15"}`}
                      >
                        {passed ? (
...
function NewRegressionModal({
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!deductTokenAction(`Create regression suite "${name.trim()}"`)) return;
    ...
    onSave(suite);
    toast.success(
      `Regression suite "${suite.name}" created with ${suite.testCaseIds.length} cases`,
    );
  }
    ...
    <Modal onClose={onClose} title="New regression suite">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <div className="max-h-48 overflow-y-auto border border-[var(--c-border)] rounded-[8px] bg-[var(--c-bg-card)]">
            {filteredCases.length === 0 ? (
              <p className="py-6 text-center text-sm text-[var(--c-text-muted)]">
              ...
              filteredCases.map((tc) => (
                ...
                >
                  <span
                    className={`inline-block h-3.5 w-3.5 rounded-sm border ${selectedCases.has(tc.id) ? "bg-[var(--c-accent)] border-[var(--c-accent)]" : "border-[var(--c-border)]"}`}
                  />
                  <span className="flex-1 truncate text-[13px] text-[var(--c-text)]">
                    {tc.title}
                  </span>
                  <span
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            type="submit"
            disabled={!name.trim() || selectedCases.size === 0}
            className="rounded-[8px] bg-[var(--c-text)] px-[16px] py-[8px] text-[13px] font-medium text-[var(--c-bg)] transition-all hover:-translate-y-[1px] hover:opacity-90 disabled:opacity-40"
          >
            Create suite
          </button>
        </div>
