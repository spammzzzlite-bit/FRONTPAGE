...
import { useRuns, useTestCases, useSuites, useProjects, useBugs } from "@/frontend/store/store";
import { PageHeader } from "./_app.projects";

export const Route = createFileRoute("/_app/analytics")({
  head: () => ({ meta: [{ title: "Analytics — QAMind AI" }] }),
...
function AnalyticsPage() {
  ...
  const scopedSuites = selectedProjectId
    ? suites.filter((s) => s.projectId === selectedProjectId)
    : suites;
  const scopedCases = selectedProjectId
    ? testCases.filter((tc) => scopedSuites.some((s) => s.id === tc.suiteId))
    : testCases;
  const scopedRuns = selectedProjectId
    ? runs.filter((r) => r.projectId === selectedProjectId)
    : runs;
  const scopedBugs = selectedProjectId
    ? bugs.filter((b) => b.project_id === selectedProjectId)
    : bugs;

  // Compute metrics — use lastRunStatus for execution results, authorStatus for authoring lifecycle
  const totalCases = scopedCases.length;
  const passedCases = scopedCases.filter((tc) => tc.lastRunStatus === "passed").length;
  const failedCases = scopedCases.filter((tc) => tc.lastRunStatus === "failed").length;
  ...
  const avgDuration =
    scopedRuns.length > 0
      ...
      : 0;
  const openBugs = scopedBugs.filter((b) => !b.is_resolved).length;

  // Pass rate trend (last 7 runs)
  const passRateTrend = scopedRuns
    .slice(0, 7)
    .reverse()
    .map((run) => {
      const p =
        run.results.length > 0
          ? Math.round(
              (run.results.filter((r) => r.status === "passed").length / run.results.length) * 100,
            )
          : 0;
  ...
  const durationTrend = scopedRuns
    .slice(0, 10)
    .reverse()
    .map((run) => ({
      run: run.id,
  ...
  // Cases by status — separate authoring and execution status for accurate reporting
  const statusData = [
    {
      name: "Draft",
      count: scopedCases.filter((tc) => (tc.authorStatus || tc.status) === "draft").length,
      color: "var(--color-muted-foreground)",
    },
    {
      name: "Ready",
      count: scopedCases.filter((tc) => (tc.authorStatus || tc.status) === "ready").length,
    ...
    {
      name: "Skipped",
      count: scopedCases.filter((tc) => tc.lastRunStatus === "skipped").length,
  ...
  const projectCoverage = displayProjects.map((p) => {
    ...
    const pPassed = pCases.filter((tc) => tc.lastRunStatus === "passed").length;
    return {
    ...
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col justify-between md:flex-row md:items-end gap-4 border-b-[2px] border-[var(--c-text)] pb-6 mt-[20px] mb-8">
        <div className="stagger-item">
          ...
          <p className="mt-2 text-[15px] text-[var(--c-text-muted)]">
            Coverage trends, performance metrics, and quality insights.
          </p>
        </div>
        <div className="stagger-item shrink-0 flex items-center gap-2">
          <label className="font-mono text-[11px] uppercase tracking-wider text-[var(--c-text-muted)]">
            Filter Project:
          </label>
          <select
      ...
      <div className="rounded-[12px] border border-[var(--c-border)] bg-[var(--c-bg-card)] p-6 md:p-10 shadow-[var(--shadow-sm)] mb-12">
        <div className="mb-8 border-b border-[var(--c-border)] pb-6">
          <p className="font-mono text-[11px] uppercase tracking-[0.12em] text-[var(--c-accent)] font-medium">
            Flaky Tests
          </p>
          <h2 className="mt-3 font-display text-[28px] text-[var(--c-text)]">
            Flakiness Leaderboard
          </h2>
          <p className="mt-2 text-[14px] text-[var(--c-text-muted)] leading-relaxed">
            Tests that passed but took &gt;3s — the ones quietly costing you.
          </p>
        </div>
        {flakyList.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <p className="mt-4 text-[14px] text-[var(--c-text-muted)]">
          ...
          <div className="overflow-hidden rounded-[8px] border border-[var(--c-border)]">
            <div className="grid grid-cols-[40px_1fr_100px] gap-4 border-b border-[var(--c-border)] bg-[var(--c-bg-hover)] px-5 py-3 font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--c-text-muted)]">
              <span>#</span>
              <span>Test case</span>
              <span>Flaky runs</span>
            </div>
            {flakyList.map((item, i) => (
              <div
                key={item.id}
                className={`grid grid-cols-[40px_1fr_100px] gap-4 px-5 py-4 text-[13px] hover:bg-[var(--c-bg-hover)] transition-colors ${i > 0 ? "border-t border-[var(--c-border)]" : ""}`}
              >
                <span className="font-mono text-[11px] text-[var(--c-text-muted)] pt-0.5">
                  {i + 1}
                </span>
                <span className="truncate font-medium text-[var(--c-text)]">{item.title}</span>
                <span className="font-mono text-[11px] text-[var(--c-fail)] font-medium pt-0.5">
                  {item.count}×
                </span>
              </div>
            ))}
...
function EmptyChart({ message }: { message: string }) {
  return (
    <div className="flex h-[200px] items-center justify-center rounded-[8px] border border-dashed border-[var(--c-border-strong)] bg-[var(--c-bg-input)] text-center">
      <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-[var(--c-text-muted)]">
        {message}
      </p>
    </div>
  );
